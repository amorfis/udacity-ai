...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        if self.x < radius:
            distance_to_center = sqrt(pow(radius - self.x, 2) + pow(radius - self.y, 2))
            cte = distance_to_center - radius

        if self.x >= radius and self.x <= 3 * radius:
            if self.orientation < pi / 2 or self.orientation > pi * 3 / 2:
                cte = self.y - 2 * radius
            else:
                cte = - self.y

        if self.x > 3 * radius:
            distance_to_center = sqrt(pow(self.x - 3*radius, 2) + pow(radius - self.y, 2))
            cte = distance_to_center - radius
