...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for i in range(0, dim+2):
        if i != 2 and i != 3:
            retain_values.append(i)

    # process the data
    for k in range(len(data)):

        Omega = Omega.expand(dim+2, dim+2, retain_values, retain_values)
        Xi = Xi.expand(dim+2, 1, retain_values, [0])
        ...
        for i in range(2, dim+2):
            prim_retain.append(i)


        Omega_prim = Omega.take(prim_retain, prim_retain)
        A = Omega.take([0, 1], prim_retain)
        B = Omega.take([0, 1], [0, 1])

        Xi_prim = Xi.take(prim_retain, [0])
        C = Xi.take([0, 1], [0])

        Omega = Omega_prim - A.transpose() * B.inverse() * A
        Xi = Xi_prim - A.transpose() * B.inverse() * C
