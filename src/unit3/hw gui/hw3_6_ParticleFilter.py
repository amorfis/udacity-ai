...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
            steering = float(motion[0])
            distance = float(motion[1])

            #noise
            steering += (2 * random.random() - 1) * self.steering_noise
            distance += (2 * random.random() - 1) * self.distance_noise

            if steering < 0.001:
                #assume move forward, ignore steering
                x = self.x + distance * cos(self.orientation)
                y = self.y + distance * sin(self.orientation)
                orientation = self.orientation + steering
            else:
    ...
    def sense(self, addNoise = 1): #do not change the name of this function
        ...
        for landmark in landmarks:
            landmarkX = landmark[1]
            landmarkY = landmark[0]
            dx = landmarkX - self.x
            dy = landmarkY - self.y
            absoluteAngle = atan2(dy, dx)
            angle = absoluteAngle - self.orientation

            if addNoise:
                angle += (2*random.random() - 1) * self.bearing_noise
