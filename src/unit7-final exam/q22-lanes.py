...
def on_road(road, x, y):
    return len(road) > x >= 0 <= y < len(road[0])


def try_replace_value(dp_grid, x, y, target_x, target_y, cost):
    value = dp_grid[target_x][target_y] + cost
    if value < dp_grid[x][y]:
        dp_grid[x][y] = value
        return True
...
def collision(road, x, y):
    return road[x][y] == 0


def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    dp_grid = [[10000 for r in range(len(road[0]))] for c in range(len(road))]
    policy = [[-1 for r in range(len(road[0]))] for c in range(len(road))]
    dp_grid[goal[0]][goal[1]] = 0
    ...
    while change:
        change = False
        for x in range(len(dp_grid)):
            for y in range(len(dp_grid[0])):
                if collision(road, x, y):
                    continue

                for i in range(len(moves)):
                    target_x = x + moves[i][0]
                    target_y = y + moves[i][1]

                    if on_road(road, target_x, target_y) and not collision(road, target_x, target_y):
                        cost = 1. / road[x][y]
                        if moves[i][0] != 0:
                            cost += lane_change_cost

                        replaced = try_replace_value(dp_grid, x, y, target_x, target_y, cost)
                        if replaced:
                            policy[x][y] = i

                        change = replaced or change
